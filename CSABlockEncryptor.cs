using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace CSA
{
    class CSABlockEncryptor
    {
        private byte[] _data;

        private const uint _DVBCSA_KEYSBUFF_SIZE = 56;
        private const uint _DVBCSA_CWBITS_SIZE = 64;
        private const uint _DVBCSA_DATA_SIZE = 8;

        private CSAKeyStruct _ks;
        private uint len;

        #region block_sbox
        private static readonly byte[] _dvbcsa_block_sbox =
            {   
                ,
            };
        #endregion
        #region block_perm
        private static readonly byte[] _csa_block_perm =
            {
                ,
            };
        #endregion

        // constructor
        public CSABlockEncryptor(CSAKeyStruct ks, byte[] data_in, uint len)
        {
            _data = data_in;

            _ks = ks;

            _len = len;

        }

        public void CSABlockEncrypt(uint startOffset)
        {
            uint i = 0;
 
            while (i < _DVBCSA_KEYSBUFF_SIZE)
            {
                byte L;
                byte S;

                S = _dvbcsa_block_sbox[_ks.sch[i] ^ _data[startOffset + 7]];

                L    = _data[startOffset + 1];

                _data[startOffset + 1] = (byte)(_data[startOffset + 2] ^ _data[startOffset + 0]);
                _data[startOffset + 2] = (byte)(_data[startOffset + 3] ^ _data[startOffset + 0]);
                _data[startOffset + 3] = (byte)(_data[startOffset + 4] ^ _data[startOffset + 0]);
                _data[startOffset + 4] = _data[startOffset + 5];
                _data[startOffset + 5] = (byte)(_data[startOffset + 6] ^ _csa_block_perm[S]);
                _data[startOffset + 6] = _data[startOffset + 7];
                _data[startOffset + 7] = (byte)(_data[startOffset + 0] ^ S);

                _data[startOffset + 0] = L;

                i++;
            }
            
        }


        public void CSAXor64 (uint offset)
        {
            uint i;

            for (i = 0; i < 8; i++)
                _data[offset + i] ^= _data[offset + i + _DVBCSA_DATA_SIZE];
        }
    }
}

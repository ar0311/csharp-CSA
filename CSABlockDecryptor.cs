using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace CSA
{
    class CSABlockDecryptor
    {
        private byte[] _data;

        private const int _DVBCSA_KEYSBUFF_SIZE = 56;
        private const uint _DVBCSA_CWBITS_SIZE = 64;
        private const uint _DVBCSA_DATA_SIZE = 8;

        private CSAKeyStruct _ks;
        private int _len;

        #region block_sbox
        private static readonly byte[] _dvbcsa_block_sbox =
            {   
                ,
            };
        #endregion
        #region block_perm
        private static readonly byte[] _csa_block_perm =
            {
                ,
            };
        #endregion


        public CSABlockDecryptor(CSAKeyStruct ks, byte[] data_in, int len)
        {
            _data = data_in;

            _ks = ks;

            _len = len;
        }

        public void CSABlockDecrypt(int startOffset)
        {
            int i = _DVBCSA_KEYSBUFF_SIZE - 1;

            while (i >= 0)
            {
                byte L;
                byte S;

                S = _dvbcsa_block_sbox[_ks.sch[i] ^ _data[startOffset + 6]];

                L = (byte)(_data[startOffset + 7] ^ S);

                _data[startOffset + 7] = _data[startOffset + 6];
                _data[startOffset + 6] = (byte)(_data[startOffset + 5] ^ _csa_block_perm[S]);
                _data[startOffset + 5] = _data[startOffset + 4];
                _data[startOffset + 4] = (byte)(_data[startOffset + 3] ^ L);
                _data[startOffset + 3] = (byte)(_data[startOffset + 2] ^ L);
                _data[startOffset + 2] = (byte)(_data[startOffset + 1] ^ L);
                _data[startOffset + 1] = _data[startOffset + 0];

                _data[startOffset + 0] = L;

                i--;
            }
        }


        public void CSAXor64(int offset)
        {
            int i;

            for (i = 0; i < 8; i++)
                _data[offset + i - 8] ^= _data[offset + i];
        }
    }
}
